...
class PartnerXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, docs):
        ...
        for rec in sales:
            ...
            if rec.approve_date:
                order_approve_days = rec.approve_date - rec.create_date.date()

            if delivery:
                if delivery.scheduled_date and rec.approve_date:
                   delivery_approve_days = rec.approve_date - rec.date_order.date()
            else:
                delivery_approve_days = 0

            if invoice:
                if invoice.invoice_date and rec.approve_date:
                    invoice_approve_days = rec.approve_date - invoice.invoice_date
            else:
                invoice_approve_days = 0
            ...
            rec_list.append({
                ...
                'delivery_date': delivery.scheduled_date if delivery.scheduled_date else '-',
        ...
        for line in rec_list:
            ...
            sheet.write(row, 3, str(line['order_date']), title2)
            sheet.write(row, 4, str(line['approval_date']),title2)
            sheet.write(row, 5, line['approval_days'],title2)
            sheet.write(row, 6, str(line['delivery_date']),title2)
            sheet.write(row, 7, line['delivery_approve_days'],title2)
            sheet.write(row, 8, str(line['invoice_date']),title2)
